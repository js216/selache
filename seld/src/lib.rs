...
// lib.rs --- seld: LDF-driven linker library for the Selache toolchain
...
/// Result of a successful link: the output image bytes, and optionally
/// the contents of a map file that the driver can write alongside it.
pub struct LinkOutput {
    pub image: Vec<u8>,
    pub map_file: Option<String>,
}

/// Run the full link pipeline for the given options and return the
/// produced bytes. The driver decides where to write them.
pub fn link(opts: &cli::Options) -> error::Result<LinkOutput> {
    // Read LDF
    ...
    let image = output::generate(&link_layout, &objects, &symtab, &config)?;

    let map_file = opts
        .map_file
        .as_ref()
        .map(|_| generate_map(&ldf, &link_layout, &symtab));

    Ok(LinkOutput { image, map_file })
}
