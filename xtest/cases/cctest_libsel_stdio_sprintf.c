...
// cctest_libsel_stdio_sprintf.c --- libsel stdio/sprintf per-function selcc
...
static void put_padding(char *buf, size_t size, size_t *pos, char padch,
                        int count)
{
   int i;
   for (i = 0; i < count; i++)
...
static void emit_number(char *buf, size_t size, size_t *pos, const char *prefix,
                        int prefix_len, const char *digits, int numlen,
...
enum length_mod { LEN_NONE, LEN_HH, LEN_H, LEN_L, LEN_LL };
...
int vsnprintf(char *buf, size_t size, const char *fmt, va_list ap)
{
   ...
   while (*fmt) {
      ...
      switch (*fmt) {
         case '%': put_char(buf, size, &pos, '%'); break;
         ...
         case 'i': {
...

            {
               ...
               emit_number(buf, size, &pos, signbuf, signlen, numbuf, numlen,
         ...
         case 'o': {
            ...
            emit_number(buf, size, &pos, prefix, prefix_len, numbuf, numlen,
                        width, flags_minus, flags_zero);
            break;
         }
