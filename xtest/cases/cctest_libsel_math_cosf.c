...
// cctest_libsel_math_cosf.c --- libsel math/cosf per-function selcc test
...
static const float _cf_half_pi = 1.57079632679489661923f;
...
float cosf(float x)
{
   ...
   if (isinf(x))
      return NAN;
   return sinf(_cf_half_pi - x);
}

int test_main(void)
{

   return (cosf(0.0f) > 0.999f && cosf(0.0f) < 1.001f) ? 0x55 : 0xAA;
}
